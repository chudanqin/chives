...
_stock_daily_query_parameters = 'code, date, open, high, low, close, preclose, volume, amount, adjustflag, turn, tradestatus, pctChg, peTTM, pbMRQ, psTTM, pcfNcfTTM, isST'
        ...
        def _tx(handler):
            try:
                ...
                handler.create_table(
                    table_name, _stock_daily_table_column_define(), table_constraint='PRIMARY KEY(ts_code)')
                for i in data.index:
                    row_data = data.loc[i].array
...
def udpate_stock_daily_from_pool():
    #### 登陆 baostock 系统 ####
    lg = bs.login()
    if lg.error_code != '0':
    ...
    db_init.execute(db_init.DB_PATH_MAIN, lambda handler: handler.create_text_table(
        db_init.DB_TABLE_STOCK_DAILY, _stock_daily_query_parameters.split(', '), table_constraint='PRIMARY KEY(code, date)'))
    ...
    for sc in stock_codes:
        ...
        def _tx1(handler):
            nonlocal row
            row = handler.fetch_first('SELECT MAX(date) from %s WHERE code = ?' % (db_init.DB_TABLE_STOCK_DAILY), [revised_code])
        db_init.execute(db_init.DB_PATH_MAIN, _tx1)

        begin_date = None
        if row is not None and len(row) == 1 and row[0] is not None:
            if row[0] >= curr_date:
                print('%s has updated already on %s' % (revised_code, row[0]))
                continue
            begin_date = date_util.date_str_from_day_delta()

        _update_stock_daily(revised_code, begin_date)
