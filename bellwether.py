        ...
        def _tx(handler1):
            try:
                ...
                handler0.create_table(
                    table_name, ['code TEXT', 'name TEXT', 'min REAL', 'max REAL', 'curr REAL', 'p REAL', 'industry TEXT'])

                for si in stock_info:
                    code = tushare_init.baostock_code(si[0])
                    min_max = handler1.fetch_first('SELECT min(cast(%s as decimal)), max(cast(%s as decimal)) FROM %s WHERE code = ? and %s is not \'\' and %s > 0' % (
                        target, target, db_init.DB_TABLE_STOCK_DAILY, target, target), [code])
                    curr = handler1.fetch_first('SELECT cast(%s as decimal) FROM %s WHERE code = ? and %s is not \'\' ORDER BY date DESC limit 1' % (
                        target, db_init.DB_TABLE_STOCK_DAILY, target), [code])
